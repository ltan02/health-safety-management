...
function Task({ id, task}) {
  ...
  return (
      <Card sx={{ boxShadow: 3, borderRadius: 2, opacity: isDragging ? 0 : 1}} id={task.id} ref={setNodeRef} style={style}>
        <CardContent>
          <Box sx={{ gap: 1 }}>
            ...
            <Typography gutterBottom variant="h7" component="div" fontWeight={600} fontSize={20} sx={{ color: '#3a3a3a' }}>
              {task.title}
            </Typography>
          </Box>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', mt: 1 }}>
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
              <IconButton size="small">
                {borderCompletion > 100 ? <LocalFireDepartmentIcon sx={{ color: 'red' }} /> : <DateRangeIcon />}
              </IconButton>
