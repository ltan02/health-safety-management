...
import LoadingBar from "../global/LoadingBar.jsx";
import useAutoScroll from "../../hooks/useAutoScroll";

function Dashboard() {
  const { filteredTasks, filterTasks, setFilteredTasks, fetchTasks , loading} =
    useTasks();

  const { forms, fetchForms, groupedByRows, sortedRows, activeForm } =
    useForm();
  const { activeId, handleDragStart, handleDragEnd } = useDragBehavior(
    filteredTasks,
    setFilteredTasks
  );
        ...
        task.comments.map((comment) => {
          ...
          const data = {
            ...
            user: employees.filter(
              (employee) => employee.id === comment.userId
            )[0],
          };
  ...
  const hasTempComment = (commentData, newComment) => {
    ...
    commentDataCopy.map((data) => {
      if (
        data.id.includes("temp") &&
        data.comment.content === newComment.comment.content
      ) {
        tempId = data.id;
        return;
      }
  ...
  useEffect(() => {
    if (activeId) {
      setColumnFlowMap(() => {
        ...
        const activeTask = filteredTasks[activeColumn].find(
        ...
        flowMap[activeTask.statusId].forEach((item) => {
          ...
          newMap[toColumn.id].push({
            ...
            statusName: statuses.find(
              (status) => status.id === activeStateMap[item.toStateId].statusId
            )?.name,
