...
import { Box, Typography, Select, MenuItem, Grid, Menu, TextField } from "@mui/material";
    ...
    return (
        ...
        >
            <Select
                ...
                displayEmpty
                renderValue={(value) => statuses.find((status) => status.id === value)?.name}
            ...
            >
                {flowMap[incident?.statusId].map(({ toStateId, transitionId }) => (
                    ...
                    >
                        ...
                        >
                            <p style={{ margin: 0, fontSize: 14 }}>{activeTransitionMap[transitionId]?.label}</p>
                            <div style={{ display: "flex", alignItems: "center" }}>
                                ...
                                >
                                    {statuses.find((status) => status.id === activeStateMap[toStateId]?.statusId)?.name}
                                </p>
                            </div>
                        </div>
                    </MenuItem>
            ...
            >
                ...
                <Box sx={{ mt: 2, px: 2, pb: 2 }}>
                    <Grid container spacing={2} sx={{ rowGap: 1 }}>
                        ...
                        <Grid item xs={6}>
                            {incident && <Typography sx={{ fontSize: "14px" }}>{incident.incidentDate}</Typography>}
                        </Grid>
                        <Grid item xs={6}>
                            <Typography fontWeight={"bold"} color={"secondary.main"}>
                                Category
                        ...
                        >
                            {incident &&
                                currentField[FIELD.EMPLOYEES_INVOLVED]
                                    .filter((employee) => employee.id !== currentField[FIELD.REPORTER].id)
                                    .map((employee) => {
                                        return (
                                            <Grid
                                                container
