...
model_basic = GenerativeModel("text-bison@001")
...
async def talk(prompt_model: Prompt):
    try:
        response = model1.generate_content(prompt_model.prompt)
        return {"response": response.text}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
def newChat(chatsession):
    ...
    is an ISO formatted string with format yyyy-MM-dd'T'HH:mm, so use the LIKE() function for strings - do not cast them
    to the DATE format. timeperiod comparisons should be made with numbers in string format. 
